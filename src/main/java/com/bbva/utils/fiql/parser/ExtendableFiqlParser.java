 ...
 * construct {@link org.apache.cxf.jaxrs.ext.search.SearchCondition} structure. Since this class operates on Java type T, not on XML
 * structures "selectors" part of specification is not applicable; instead selectors describes getters of type
 * T used as search condition type (see {@link ExtendedSearchCondition#isMet(Object)} for details.
 *
...
public class ExtendableFiqlParser<T> {
     ...
     */
    public ExtendableFiqlParser(Class<T> tclass) {
     ...
     * @return tree of {@link org.apache.cxf.jaxrs.ext.search.SearchCondition} objects representing runtime search structure.
     * @throws org.apache.cxf.jaxrs.ext.search.FiqlParseException when expression does not follow FIQL grammar
     */
    ...
    private Comparison parseComparison(String expr) throws FiqlParseException {
        String comparators = StringUtils.join(operatorsMap.keySet(), "|");
    ...
    public static void addOperator(String operator, ConditionType conditionType) {
        ...
        public SearchCondition<T> build() throws FiqlParseException {
            ...
            if (!hasSubtree && AND.equals(operator)) {
                try {
                    ...
                    for (ASTNode<T> node : subnodes) {
                        ExtendableFiqlParser<T>.Comparison comp = (Comparison) node;
